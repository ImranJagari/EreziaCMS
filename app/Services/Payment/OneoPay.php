...
class OneoPay extends Payment
{
    ...
    public function __construct()
    {
        ...
        {
            $url = config('dofus.payment.oneopay.url');
            $url = str_replace('{KEY}', config('dofus.payment.oneopay.key'), $url);
            $json = json_decode(file_get_contents($url));

            Cache::add('payment.oneopay', $json, self::CACHE_EXPIRE_MINUTES);
        }
    ...
    public function check($palier, $code)
    {
        ...
        $key        = config('dofus.payment.oneopay.key');
        $validation = config('dofus.payment.oneopay.validation');
        ...
        $check->provider = config('dofus.payment.oneopay.name');
